...
innerDimensionsBoxPlusLid(        // Dimensions are inner dimensions of the box
    [72,65,41],               // Dimensions of the box. Latch is along X axis
    ...
    translate([37,105,0])lattice(11,10);
    translate([37,-432,20])rotate([90,0,0])lattice(11,5);
    translate([37,34,20])rotate([0,90,0])lattice(5,10);
}
module lattice(a, b){
    for(i=[-a:a]){
        for(j=[-b:b]){
  translate([3*i, 3*j, 0])cylinder(1000, 0.75, 0.75, center=true); 
        } 
    }
}
